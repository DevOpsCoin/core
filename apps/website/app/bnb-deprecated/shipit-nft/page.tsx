...
export default function ShipItNFTPage() {
  ...
  const mintPrice = mintPriceData
    ? (Number(mintPriceData) / 1e18).toString()
    : null;
  ...
  // ✅ Stable callback, safe for useEffect deps
  ...
  // ✅ Add loadOwnedNFTs to the dependency list
  useEffect(() => {
    if (txSuccess) {
  ...
  }, [txSuccess, txError, loadOwnedNFTs]);
  ...
  return (
    <section className="container max-w-4xl mx-auto mt-12 p-6 bg-gray-900 rounded-xl border border-cyan-700/40 shadow-lg text-center">
      <h1 className="text-3xl font-bold text-cyan-400 mb-4">
        Ship-It Badge — The First Commit
      </h1>
      <p className="text-gray-300 mb-6">
        Mint your limited-edition <strong>The First Commit</strong> badge — the
        very first entry in the <strong>Ship-It Badge</strong> series. Each mint
        supports the{' '}
      ...
      <div className="bg-gray-800 border border-cyan-700/40 rounded-lg shadow-inner p-6 text-left mb-10">
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div>
            ...
            <p className="text-gray-300 mb-3">
              This badge marks the <strong>founding supporters</strong> of
              DevOpsCoin — those who believed in the mission from the start.
              Only a limited number will ever be minted.
            </p>
            <p className="text-gray-400 mb-3">
              Each on-chain mint is an immutable proof of early contribution to
              the DevOps culture and the Ship-It Fund. A digital artifact for
              the engineers who <em>ship first</em>.
            </p>
        ...
        <div className="text-center mt-8">
          <a
            href="#mint-section"
            className="nav-btn glow-cyan text-lg px-6 py-3 inline-block"
          >
      ...
      <div id="mint-section" className={isConnected ? 'mt-8' : 'hidden mt-8'}>
        ...
        <button
          onClick={() => {
            ...
            mintBadge?.({
              ...
              value: mintPriceData
                ? BigInt(mintPriceData as string)
                : undefined,
            });
          }}
        ...
        {mintError && (
          <p className="text-red-400 text-sm mt-2">{mintError.message}</p>
        )}
      </div>
