    ...
    async (cancelledRef?: { current: boolean }) => {
      ...
      try {
        ...
        try {
          ...
          const subgraphData: unknown = await subgraphRes.json()
          const pair = (subgraphData as { data?: { pairs?: { token0Price?: string }[] } })?.data
            ?.pairs?.[0]
          if (pair?.token0Price) {
        ...
        const bnbData: unknown = await bnbRes.json()
        const bnbUsd = (bnbData as { binancecoin?: { usd?: number } })?.binancecoin?.usd ?? 600 // fallback to 600 if API fails
